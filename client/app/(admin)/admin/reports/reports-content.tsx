...
export function AdminReportsContent() {
  ...
  const preview = useReportPreview(appliedFilters);
  ...
  const applyFilters = useCallback(() => {
    ...
    router.replace(`/admin/reports${params.toString() ? `?${params.toString()}` : ""}`);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Reports"
        description="Generate filtered academic reports with preview and PDF export."
      />
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Button
            onClick={() => downloadInstitutionalPdf({ filters: appliedFilters })}
